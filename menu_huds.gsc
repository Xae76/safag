drawHuds()
{
    ...
    //Text Huds
    //self.Hud.MenuName      = self createText("default", 2, "TOPCENTER", "TOPCENTER", -100, 65, 1, 1, "SeeK HQ", ( 1, 1, 1 ) );
...
option_info(index, hud_index, override)
{
    ...
    if(index < 0 || hud_index > 10 || !isDefined(index) || !isDefined(hud_index))
        return;
    ...
    if(!isDefined(self.Hud.Bools[hud_index]))
        self.Hud.Bools[hud_index] = createRectangle("CENTER", "CENTER", -10, 0, 10, 10, (1,1,1), "white", 9, 0);

    yPos = -81 + (14.4 * hud_index);
    if(self isBoolOption(menu, index) && !self isSubMenu(menu, index) && !self isSliderOption(menu, index))
    {
        off_color = (1, 0, 0);
        toggle_color = (1, 1, 1);
        self.Hud.Bools[hud_index].y = yPos;
        if(self.Menu.Bool[menu][index] == true)
        {
            self.Hud.Bools[hud_index] setShader("thumbsup", 10, 10);
            self.Hud.Bools[hud_index].color = toggle_color;
            self.Hud.Bools[hud_index].alpha = 1;
        }
        else
        {
            self.Hud.Bools[hud_index] setShader("menu_mp_lobby_locked", 10, 10);
            self.Hud.Bools[hud_index].color = toggle_color;
            self.Hud.Bools[hud_index].alpha = 1;
        }
    }
    else if(self isSubMenu(menu, index) && !self isBoolOption(menu, index) && !self isSliderOption(menu, index))
    {
        self.Hud.Bools[hud_index].y = yPos;
        self.Hud.Bools[hud_index] setShader("white", 10, 10);
        self.Hud.Bools[hud_index].alpha = 1;
    }
    else if(self isSliderOption(menu, index) && !self isBoolOption(menu, index) && !self isSubMenu(menu, index))
    {
        self.Hud.Bools[hud_index].y = yPos;
        self.Hud.Bools[hud_index] setShader("white", 10, 10);
        self.Hud.Bools[hud_index].alpha = 1;
    ...
    {
        self.Hud.Bools[hud_index].alpha = 0;
    }
}
